...
function getUsuario($id){
	...
	$pstmt = $con->prepare("SELECT U.IDUSUARIO,U.NOMBRES,U.APELLIDOS,U.CORREO_INSTITUCIONAL,U.CORREO_ALTERNO,U.NUMERO_CELULAR,
							U.NUMERO_TEL_ALTERNO,U.CUENTA_SKYPE,I.NOMBRE_INSTITUCION,U.MESES_TERMINAR,U.COMPROMISO,P.NOMBRE
							FROM USUARIO U, PERMISO P, INSTITUCION I WHERE U.ESTADO =1 AND U.IDPERMISO=P.IDPERMISO AND I.IDINSTITUCION=U.IDINSTITUCION AND U.IDUSUARIO=?");
	$pstmt->execute(array($id));
	$IDUSUARIO = $pstmt->fetch(PDO::FETCH_ASSOC);
	echo json_encode($IDUSUARIO);
}
