function validarVisibilidadTabla(data){
	...
	result = $.grep(data, function(e){ return e.IDFUNCIONALIDAD === "21"; });
	if (result.length === 0){
		$(location).attr('href','../../front/Seguridad/errorPermiso.html');
	}else{ 
		//permisover = $.grep(data, function(e){ return e.IDFUNCIONALIDAD === "17"; });
		permisoreg = $.grep(data, function(e){ return e.IDFUNCIONALIDAD === "22"; });
		permisomod = $.grep(data, function(e){ return e.IDFUNCIONALIDAD === "23"; });
		permisodel = $.grep(data, function(e){ return e.IDFUNCIONALIDAD === "24"; });
		...
		if (permisomod.length === 0) {
			$('a.modificar-autor').attr("style","visibility: hidden");
		}
		if (permisodel.length === 0) {
			$('a.eliminar-autor').attr("style","visibility: hidden");
		}
	}	
}
